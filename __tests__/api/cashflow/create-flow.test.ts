...
describe("/api/cashflow/create-flow", () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it("deve criar um novo cash flow e realocar com sucesso (cenário positivo)", async () => {
    const flowData = {
      "10": { receitaOrcada: 10000.5, despesaOrcada: 8000.75 },
      "11": { receitaOrcada: 12000.25, despesaOrcada: 9000.5 },
  ...
  it("deve rejeitar a criação se não houver meses válidos", async () => {
    ...
    expect(JSON.parse(res._getData())).toEqual({
      ...
      error: expect.stringContaining(
        "Apenas meses do mês atual até dezembro são permitidos"
      ),
    })
  ...
  it("deve lidar com erro na criação do fluxo", async () => {
    ...
    ;(verifyToken as jest.Mock).mockResolvedValue(true)
    ;(prisma.orcamento.findFirst as jest.Mock).mockResolvedValue(null)
    ;(prisma.orcamento.createMany as jest.Mock).mockRejectedValue(
      new Error("Erro ao criar fluxo")
    )
