...
import handler from '@/pages/api/budget/get-monthly';
...
describe('/api/budget/get-monthly', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('deve lidar com erro interno do servidor', async () => {
    ...
    (verifyToken as jest.Mock).mockResolvedValue(true);
    (prisma.orcamento.findUnique as jest.Mock).mockRejectedValue(new Error('Erro de banco de dados'));
